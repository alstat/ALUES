...
List case_c(NumericMatrix df, NumericMatrix score, CharacterMatrix suiClass, double Min, double Max, double Mid, double mfNum,
            double bias, double wt, int j, double a, double b, double c, double d, double e, double f, double l1, double l2, double l3, double l4, double l5, double sigma) {
  int i, w = j - 1;
  List out(2);
    ...
    if (mfNum == 1) {
      if ((df(i, w) <= Min) || (df(i, w) >= Max)) {
        ...
        if (wt == 0) {
          ...
          if (bias == 1) {
            l1 = 0; l2 = 1 - (f / Max); l3 = e / Max; l4 = 1 - (d / Max); l5 = Max / Max;
          }
          if ((score(i, w) >= l1) && (score(i, w) < l2)) {
            suiClass(i, w) = "N";
          } else if ((score(i, w) >= l2) && (score(i, w) < l3)) {
    ...
    if (mfNum == 2) {
      if ((df(i, w) <= Min) || (df(i, w) > Max)) {
        ...
        if (wt == 0) {
          ...
          if (bias == 1) {
            l1 = 0; l2 = 1 - (c / Max); l3 = 1 - (b / Max); l4 = l5 = 1;
          }
          if ((score(i, w) >= l1) && (score(i, w) < l2)) {
            suiClass(i, w) = "N";
          } else if ((score(i, w) >= l2) && (score(i, w) < l3)) {
    ...
    if (mfNum == 3) {
      if (df(i, w) <= Mid) {
        ...
        if (wt == 0) {
          score(i, w) = exp(-(1 / 2) * pow(((df(i, w) - Mid) / sigma), 2));
          if (bias == 1) {
            l1 = 0; l2 = exp(-(1 / 2) * pow(((f - Mid) / sigma), 2));
            l3 = exp(-(1 / 2) * pow(((e - Mid) / sigma), 2));
            l4 = exp(-(1 / 2) * pow(((d - Mid) / sigma), 2));
            l5 = 1;
          }
          if ((score(i, w) >= l1) && (score(i, w) < l2)) {
            suiClass(i, w) = "N";
          ...
          if ((score(i, w) >= l1) && (score(i, w) < l2)) {
            score(i, w) = wt * ((exp((-1 / 2) * pow(((f - Mid) / sigma), 2))) / 3);
            suiClass(i, w) = "N";
          } else if ((score(i, w) >= l2) && (score(i, w) < l3)) {
            score(i, w) = wt * ((exp((-1 / 2) * pow(((e - Mid) / sigma), 2)) - exp((-1 / 2) * pow(((f - Mid) / sigma), 2))) / 3) + exp((-1 / 2) * pow(((f - Mid) / sigma), 2));
            suiClass(i, w) = "S3";
          } else if ((score(i, w) >= l3) && (score(i, w) < l4)) {
            score(i, w) = wt * ((exp((-1 / 2) * pow(((d - Mid) / sigma), 2)) - exp((-1 / 2) * pow(((e - Mid) / sigma), 2))) / 3) + exp((-1 / 2) * pow(((e - Mid) / sigma), 2));
            suiClass(i, w) = "S2";
          } else if ((score(i, w) >= l4) && (score(i, w) <= l5)) {
            score(i, w) = wt * ((1 - exp((-1 / 2) * pow(((d - Mid) / sigma), 2))) / 3) + exp((-1 / 2) * pow(((d - Mid) / sigma), 2));
            suiClass(i, w) = "S1";
          }
        }
      }
