...
List case_d(NumericMatrix df, NumericMatrix score, CharacterMatrix suiClass, double Min, double Max, double Mid, double mfNum,
            double bias, double wt, int j, double a, double b, double c, double d, double l1, double l2, double l3, double l4, double l5, double sigma) {
  int i, w = j - 1;
  List out(2);
  ...
  for (i = 0; i < df.nrow(); ++i) {
    ...
    if (mfNum == 1) {
      if ((df(i, w) <= Min) || (df(i, w) > Max)) {
        ...
        if (wt == 0) {
          if (bias == 1) {
            l3 = ((2 * Mid) - Max) / Mid; l4 = ((2 * Mid) - d) / Mid; l5 = Max / Max;
          }
          score(i, w) = (((Max - df(i, w)) / (Max - Mid)) * (1 - l3)) + l3;
    ...
    if (mfNum == 2) {
      if ((df(i, w) <= Min) || (df(i, w) > Max)) {
        ...
        if (wt == 0) {
          ...
          if (bias == 1) {
            l3 = ((2 * d) - Max) / d; l4 = l5 = 1;
          }
          score(i, w) = (((Max - df(i, w)) / (Max - d)) * (1 - l3)) + l3;
          if ((score(i, w) >= l3) && (score(i, w) < l4)) {
            suiClass(i, w) = "S2";
          } else if (score(i, w) == l4) {
            suiClass(i, w) = "S1";
    ...
    if (mfNum == 3) {
      if (df(i, w) <= Mid) {
        ...
        if (wt == 0) {
          score(i, w) = exp(-(1 / 2) * pow(((df(i, w) - Mid) / sigma), 2));
          if (bias == 1) {
            l3 = exp(-(1 / 2) * pow(((Max - Mid) / sigma), 2));
          ...
          if ((score(i, w) >= l3) && (score(i, w) < l4)) {
            score(i, w) = wt * ((exp((-1 / 2) * pow(((d - Mid) / sigma), 2)) - exp((-1 / 2) * pow(((Max - Mid) / sigma), 2))) / 3) + exp((-1 / 2) * pow(((Max - Mid) / sigma), 2));
            suiClass(i, w) = "S2";
          } else if ((score(i, w) >= l4) && (score(i, w) <= l5)) {
