#' Onion water requirement for land evaluation
    #' 
    #' A dataset containing the water characteristics of the crop requirements for farming Onion.
    #' 
    #' @details 
    #' The following are the factors for evaluation: 
#'
    #' \itemize{
#' \item WgAv - Precipitation of growing cycle (mm)
#' \item TmAvDlen3 - Daylength (h) during yield form. Period
#' }
    #' @seealso 
    #' \itemize{
    #'  \item Yen, B. T., Pheng, K. S., and Hoanh, C. T. (2006). \emph{LUSET: Land Use Suitability Evaluation Tool User's Guide}. International Rice Research Institute.
    #'  }
    #' 
    #' @docType data
    #' @keywords dataset
    #' @format A data frame with 2 rows and 8 columns
#' @name ONIONWater
NULL