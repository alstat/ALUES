...
water_suit
```
Setting the `sow_month=1` indicates that the factors for `RICEBRWater`'s `WmAv1` to January, `WmAv2` to February, `WmAv3` to March, and `WmAv4` to April. Thus, the only factors that were targetted by the `water_input` is April. Thus, setting the `sow_month=3`, would make `WmAv1` of `RICEBRWater` is March, `WmAv2` is April, etc. This in turn targets the months April to Jun.
```{r}
