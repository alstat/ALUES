...
```
While ALUES can generate tables of suitability scores and classes, it would be best to look at it from a big picture by visualizing the scores and classes computed through maps. The main requirement of course in here is the availability of the longitude and latitude of each land unit. This is possible for the land units of the region of Marinduque which has spatial locations in the encoded dataset that comes with ALUES.
...
knitr::include_graphics("../vignettes/img/scores.jpg")
```
...
```
```{r, out.width="100%", echo=FALSE}
knitr::include_graphics("../vignettes/img/classes.jpg")
```