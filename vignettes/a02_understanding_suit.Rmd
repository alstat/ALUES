...
head(banana_suit[["soil"]][["Suitability Class"]])
```
By setting the `interval="unbias"`, will generate a different likely unequally spaced suitability class intervals. Notice how the suitability class have changed for CFragm. This is because, unlike the default interval mentioned earlier, the unbias interval takes into account the shape of the membership function and the intervals of the parameter intervals. *See* Article 2 (Methodology used in ALUES).
