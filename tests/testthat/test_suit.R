...
test_that("suit: catch terrain", expect_equal(out[["terrain"]]$`Actual Factors Evaluated`, "Flood"))
test_that("suit: catch soil", expect_equal(out[["soil"]]$`Actual Factors Evaluated`, c("CFragm", "CECc", "BS", "SumBCs", "pHH2O", "OC")))

out <- suit("ricebr", water=LaoCaiWater, sow_month=1)
test_that("suit: catch water", expect_equal(out[["water"]]$`Actual Factors Evaluated`, c("Jan", "Feb", "Mar", "Apr")))

out <- suit("ricebr", temp=LaoCaiTemp, sow_month=1)
test_that("suit: catch temp", expect_equal(out[["temp"]]$`Actual Factors Evaluated`, "Feb"))
